...
class TraceMessage:
    ...
    req_ctx: Optional[TraceReqContextAsync] = None
    ...
    action_process = {
        ...
        TraceAction.TRACE_REQ_FINISH: "_trace_req_finish_process",
    ...
    def _trace_req_start_process(self):
        ...
        rid = self.rids[0]
        TraceReqContext.trace_set_proc_propagate_context(
            self.req_ctx, self.propagate_ctx
        )
        trace_context_table.put(rid, self.req_ctx)

    def _trace_req_finish_process(self):
        assert len(self.rids) == 1
        rid = self.rids[0]
        req_ctx = trace_context_table.pop(rid)
        if req_ctx is not None:
            TraceReqContext.abort(req_ctx)
    ...
    def _trace_slice_start_process(self):
        for rid in self.rids:
            ...
            if not req_ctx:
                continue
            TraceReqContext.trace_slice_start(
                req_ctx,
    ...
    def _trace_slice_end_process(self):
        for rid in self.rids:
            ...
            if not req_ctx:
                continue
            TraceReqContext.trace_slice_end(
                req_ctx,
                name=self.name,
                ts=self.ts,
                attrs=self.attrs,
    ...
    def _trace_add_attrs_process(self):
        for rid in self.rids:
            ...
            if not req_ctx:
                continue
            TraceReqContext.trace_slice_add_attr(req_ctx, self.attrs)
...
class TraceReqContextAsync(TraceReqContext):
    ...
    def trace_req_abort(self, ts=None, abort_info: Optional[Dict] = None):
        if not self.tracing_enable:
            return
    ...
    def trace_slice_start(
        ...
        if not self.tracing_enable:
            return
        ts = ts or get_cur_time_ns()
        virt_id = virt_id or uuid.uuid4().int
...
def async_trace_worker(ipc_name, otlp_endpoint, server_name, init_event):
    ...
    receiver_socket.connect(ipc_name)
...
def process_tracing_init(otlp_endpoint, server_name):
    ...
    sender_socket = zmq.Context().socket(zmq.PUSH)
    sender_socket.connect(ipc_name)
