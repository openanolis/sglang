...
def build_otel_span_tree(otel_spans):
    ...
    virt_id_map = {
        span["attributes"]["virt_id"]: span
        for span in otel_spans
        if "virt_id" in span.get("attributes", {})
    }
    for span in otel_spans:
        span["child"] = []
    ...
    for span in otel_spans:
        span_id = span["spanId"]
        # judge if root span by attributes "module"
        ...
        if "prev_span_virt_id" in span.get("attributes", {}):
            prev_span_virt_id = span["attributes"]["prev_span_virt_id"]
            if prev_span_virt_id in virt_id_map:
                prev_span = virt_id_map[prev_span_virt_id]
...
def generate_perfetto_links(engine_root_spans, smg_otel_spans):
    ...
    for root_span in engine_root_spans:
        if "parentSpanId" in root_span and root_span["parentSpanId"] in span_id_map:
            ...
            root_span["spans"][0]["link"] = parent_span

        for span in root_span["spans"]:
            span["perfetto_links"] = []

            if "link" in span:
                link_perfetto_span = span["link"]["perfetto_span"]
